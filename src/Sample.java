...
public class Sample {
	public static void main(String[] args) {
		...
		Double SqFilAve = newList.stream().map(e -> e * e).filter(e -> e > 100).mapToInt(e -> e).average()
				.getAsDouble();
		...
		List<Integer> EvenResList = ls.stream().filter(e -> e % 2 == 0).collect(Collectors.toList());
		System.out.println("Even numbers" + EvenResList);

		Map<Boolean, List<Integer>> response = ls.stream().collect(Collectors.partitioningBy(n -> n % 2 == 0));
		...
		List<Integer> numWith2 = allNum.stream().map(e -> String.valueOf(e)).filter(e -> e.startsWith("2"))
		...
		Set<Integer> onlyDup = dupList.stream().filter(e -> Collections.frequency(dupList, e) > 1)
				.collect(Collectors.toSet());
		System.out.println("Duplicates:" + onlyDup);

		Set<Integer> hs = new HashSet<>();
		Set<Integer> dupRes = dupList.stream().filter(e -> hs.add(e)).collect(Collectors.toSet());
